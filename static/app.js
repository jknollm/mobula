...
const COLOR_RAMPS = {
  ...
  // Sampled from ehtplot `afmhot_u.ctab` (GPL-3.0 project):
...
// Backward compatibility for older saved states that used "ehtplot".
COLOR_RAMPS.ehtplot = COLOR_RAMPS.afmhot_u;
...
function createDefaultAxisSettings() {
  const out = {};
  for (const dim of AXIS_CONTROL_DIMS) out[dim] = { flip: false, length: null };
  return out;
}
...
function axisSetting(dim) {
  ...
  if (!Object.prototype.hasOwnProperty.call(state.axisSettings, dim)) {
    state.axisSettings[dim] = { flip: false, length: null };
  }
...
function axisSettingsCustomizedCount() {
  ...
  for (const dim of AXIS_CONTROL_DIMS) {
    ...
    if (cfg.flip || Number.isFinite(cfg.length)) count += 1;
  }
  return count;
}
...
function axisPlaneSwapCustomizedCount() {
  let count = 0;
  for (const key of PLANE_KEYS) {
    if (axisPlaneSwapEnabled(key)) count += 1;
  }
  return count;
}
...
function fmtAxisTick(axis, unit, v) {
  ...
  if (axis === "nu" || unit === "Hz") return (displayV / 1.0e9).toFixed(3);
...
function updateHoverReadout() {
  ...
  if (state.coordSystem === "pixel") {
    ...
    if (xType === "ra") {
      const deg = unitToDegrees(xCoordDisplay, xUnit);
    ...
    if (yType === "dec") {
      const deg = unitToDegrees(yCoordDisplay, yUnit);
    ...
    lines = [
      "Plane Inspect",
      `${axisDisplayLabel(probe.planeX)} [${xUnit || "-"}]: ${xValue}`,
      `${axisDisplayLabel(probe.planeY)} [${yUnit || "-"}]: ${yValue}`,
    ];
  }
...
function renderAxisSwapRows() {
  ...
  const activePlane = PLANE_KEYS.includes(state.plane) ? state.plane : "xy";
  for (const planeKey of PLANE_KEYS) {
    const base = PLANE_OPTIONS[planeKey];
    if (!base) continue;
    const row = document.createElement("label");
    row.className = "axisSwapRow";
    row.classList.toggle("isCurrent", planeKey === activePlane);

    const checkbox = document.createElement("input");
    checkbox.type = "checkbox";
    checkbox.checked = axisPlaneSwapEnabled(planeKey);
    const canSwap = axisSize(base.planeX) > 1 && axisSize(base.planeY) > 1;
    checkbox.disabled = !canSwap;
    checkbox.addEventListener("change", async () => {
      axisPlaneSwapSet(planeKey, checkbox.checked);
      await applyAxisPlaneSwapChange(planeKey);
    ...
    const activeHint = planeKey === activePlane ? " (active plane)" : "";
    text.textContent = `Swap ${dimX}/${dimY}${activeHint}`;
...
function renderAxisSettingsDialog() {
  ...
  for (const dim of dims) {
    ...
    header.textContent = `${axisDisplayLabel(dim)} (${unit})`;
    row.appendChild(header);

    const meta = document.createElement("div");
    meta.className = "axisSettingsMeta";
    meta.textContent = `N=${axisSize(dim)}`;
    ...
    flipLabel.className = "axisSettingsCheckbox";
    const flipInput = document.createElement("input");
    flipInput.type = "checkbox";
    flipInput.checked = cfg.flip === true;
    flipInput.addEventListener("change", () => {
      axisSetting(dim).flip = flipInput.checked;
      applyAxisSettingsChange();
    });
    const flipText = document.createElement("span");
    flipText.textContent = "Revert axis";
    flipLabel.append(flipInput, flipText);
    row.appendChild(flipLabel);
...
function normalizeRenderAxisForData() {
  const options = ["t", "nu", RENDER_AXIS_HIDDEN, SAMPLE_MORPH_AXIS];
...
function canRenderSweepAxis(axisToken) {
  ...
  if (normalizedAxis === SAMPLE_MORPH_AXIS) {
    return isSampleMorphMode() && sampleCount() > 1;
  }
...
function renderSettingsSummaryText() {
  ...
  const axisLabel = axis === SAMPLE_MORPH_AXIS ? "Sample Morph" : axisDisplayLabel(axis);
...
async function runOfflineRenderFromDialog() {
  ...
  const fps = normalizeRenderFps(state.renderMoviePrefs.fps);
  const baseFramePositions = axis === SAMPLE_MORPH_AXIS ? [] : computeRenderFramePositions(axis);
  const baseFrameCount = axis === SAMPLE_MORPH_AXIS ? computeSampleMorphRenderFrameCount(fps) : baseFramePositions.length;
  if (!baseFrameCount) {
...
function setAxisRangeLabels(minEl, maxEl, axis, profile) {
  if (profile && Array.isArray(profile.coords) && profile.coords.length >= 1 && (axis === "t" || axis === "nu")) {
    ...
    minEl.textContent = fmtPhysical(axis, leftCoord, dimUnit(axis));
    maxEl.textContent = fmtPhysical(axis, rightCoord, dimUnit(axis));
    return;
  }
